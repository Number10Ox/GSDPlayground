/**
 * Test Town Data - "Bridal Falls"
 *
 * A complete hardcoded town for development and testing.
 * Provides 5 NPCs with knowledge facts, a 4-level sin chain,
 * conflict thresholds, and location assignments.
 *
...
export const TEST_SIN_CHAIN: SinNode[] = [
  {
    id: 'sin-pride',
    level: 'pride',
...
/**
 * getTopicsForNPC - Returns the available topics for a given NPC,
 * filtered by what the player has discovered and their current location.
 */
export function getTopicsForNPC(
  ...
  const npc = TEST_NPCS.find(n => n.id === npcId);
  if (!npc) return [];

  // Always available topics
  const topics: Topic[] = [
    { id: `${npcId}-greeting`, label: 'greeting', available: true },
    { id: `${npcId}-the-town`, label: 'the-town', available: true },
  ];

  // Discovery-unlocked topics
  if (discoveredSinIds.includes('sin-injustice') || discoveredSinIds.includes('sin-pride')) {
    topics.push({
      id: `${npcId}-steward-decree`,
      label: 'steward-decree',
      available: true,
      requiresDiscovery: 'sin-injustice',
    });
  }

  if (discoveredSinIds.includes('sin-theft')) {
    topics.push({
      id: `${npcId}-medicine-theft`,
      label: 'medicine-theft',
      available: true,
      requiresDiscovery: 'sin-theft',
    });
  }

  if (discoveredSinIds.includes('sin-sickness')) {
    topics.push({
      id: `${npcId}-the-sickness`,
      label: 'the-sickness',
      available: true,
      requiresDiscovery: 'sin-sickness',
    });
  }

  // Location-specific topics (only available when player is at NPC's location)
  const locationTopics: Record<string, { id: string; label: string; locationOnly: string }> = {
    'steward-ezekiel': { id: 'ez-chapel-doctrine', label: 'chapel-doctrine', locationOnly: 'church' },
    'sister-martha': { id: 'martha-store-remedy', label: 'store-remedy', locationOnly: 'general-store' },
    'brother-thomas': { id: 'thomas-farm-crops', label: 'farm-crops', locationOnly: 'homestead' },
    'sister-ruth': { id: 'ruth-well-water', label: 'well-water', locationOnly: 'well' },
    'sheriff-jacob': { id: 'jacob-office-records', label: 'office-records', locationOnly: 'sheriffs-office' },
  };

  const locTopic = locationTopics[npcId];
  if (locTopic && currentLocation === locTopic.locationOnly) {
    topics.push({
      id: locTopic.id,
      label: locTopic.label,
      available: true,
      locationOnly: locTopic.locationOnly,
    });
  }
