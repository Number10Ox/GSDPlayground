...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/generators/validators/npcStakesValidator.ts"
      provides: "NPC stakes and connectivity validation"
...
- Export `validateNPCStakes(town: TownData): ValidationResult`:
  1. For each NPC, assert `npc.knowledge` exists before accessing facts (the field is optional in the type but always populated by the generator). If knowledge is missing, emit an error.
  2. Check every NPC with knowledge has at least one fact with a sinId (NPC has stakes in some sin)
  3. Check every sin has 2+ NPCs in its linkedNpcs array
  4. Check NPC connectivity using implicit approach: every NPC must share at least one sinId with at least one other NPC (via their linkedNpcs arrays on the sin chain). For each NPC, collect all sinIds they appear in, then verify overlap with at least one other NPC's sinIds. No explicit relationship edges needed - shared sin assignments prove connectivity.
  5. Check every NPC with knowledge has at least one fact at trust level 0 (greeting/entry point)
  6. Warning: NPCs with >80% of facts at trust 60+ (too secretive, hard to crack)
  </action>
  <verify>
`npx tsc --noEmit` passes. Validators are designed to catch known failure patterns. bridalFalls (hand-crafted reference) is used as a sanity check to confirm validators don't reject known-good data: `validateSinChainDiscoverable(bridalFalls)` returns `{ valid: true }`. Test with a deliberately broken town (all facts trust 100): returns `{ valid: false, errors: [...] }`.
  </verify>
  <done>Sin chain validator catches unreachable sins and circular dependencies. NPC validator catches stakeless NPCs and disconnected NPCs (via implicit sin-sharing check).</done>
</task>
...
- This pattern ensures the exported function always returns a playable town or throws explicitly
  ...
  <verify>
`npx tsc --noEmit` passes. Validators are designed to catch known failure patterns. bridalFalls (hand-crafted reference) is used as a sanity check to confirm validators don't reject known-good data: `validateTown(bridalFalls)` returns `{ valid: true }`. `generateValidTown({ seed: "test" })` returns a town where `validateTown(result).valid === true`. Verify retry logic: mock a generator that fails first N times, confirm it retries and eventually succeeds.
  </verify>
...
- `validateTown(bridalFalls)` returns valid: true (validators don't reject known-good data)
...
- NPC connectivity checked via shared sin assignments (no explicit relationship edges needed)
- Combined validator runs all checks and merges results
- Generate-validate loop guarantees playable output
- Existing bridalFalls town passes all validation (sanity check for known-good data)
