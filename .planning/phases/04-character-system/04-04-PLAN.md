...
must_haves:
  ...
  key_links:
    ...
    - from: "src/components/Conflict/FalloutReveal.tsx"
      ...
      via: "calls useCharacter() directly to dispatch ADD_TRAIT"
      pattern: "useCharacter\\(\\).*dispatch.*ADD_TRAIT"
    ...
    - from: "src/components/Conflict/ConflictView.tsx"
      to: "src/components/Character/RelationshipDice.tsx"
      via: "passes npcId from state.activeConflict to RelationshipDice"
      pattern: "RelationshipDice.*npcId"
---
...
<task type="auto">
  ...
  <action>
    ...
    - Import useCharacter hook at component level: `const { dispatch: charDispatch } = useCharacter();`
    - FalloutReveal is a component that receives falloutDice and onComplete callback
    - After fallout severity is calculated and shown (in the VERDICT phase of the reveal sequence):
      - If severity is MINOR, SERIOUS, or DEADLY:
        ...
        - Dispatch ADD_TRAIT to character context via charDispatch: `charDispatch({ type: 'ADD_TRAIT', trait: newTrait })`
      - Show trait gain as part of the fallout reveal animation (after severity shown, before dismissal)
    - This approach has FalloutReveal calling useCharacter() directly rather than receiving dispatch as a prop, because FalloutReveal already manages its own internal state machine for the reveal sequence
  ...
  <done>MINOR/SERIOUS/DEADLY fallout creates a trait (d4/d6/d8). Trait appears in character's trait list with source: 'fallout'. Player sees "gained trait" message during fallout reveal. FalloutReveal uses useCharacter() directly to dispatch ADD_TRAIT.</done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    - Extract npcId from game state: when conflict is ACTIVE, read `gameState.activeConflict?.npcId` (from useGameState hook which is already imported)
    - Pass npcId to RelationshipDice as conflictNpcId prop
    - Render RelationshipDice alongside TraitAndItemInvocation during player's turn (below the trait/item invocation section in the center column)
    - Pass character relationships (from useCharacter), conflictNpcId, and usedRelationships from conflict state
  ...
  <done>Relationships provide dice during conflicts. Direct relationship with conflict NPC highlighted as relevant. Each relationship usable once per conflict. Relationship dice added to player pool. ConflictView extracts npcId from gameState.activeConflict and passes to RelationshipDice.</done>
</task>
...
- ConflictView correctly wires npcId from gameState.activeConflict to RelationshipDice
</success_criteria>
