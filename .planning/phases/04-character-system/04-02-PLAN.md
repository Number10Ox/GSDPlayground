...
must_haves:
  ...
  key_links:
    ...
    - from: "src/types/game.ts"
      to: "GameAction union"
      via: "START_CYCLE accepts optional dicePool"
      pattern: "START_CYCLE.*dicePool"
---
...
<task type="auto">
  <name>Task 2: GameAction type update, character creation UI, and stat-based dice pool</name>
  <files>src/types/game.ts, src/components/Character/CharacterCreation.tsx, src/utils/dice.ts, src/hooks/useGameState.tsx</files>
  <action>
    STEP 1 - Update GameAction union in `src/types/game.ts`:
    - Change START_CYCLE from `{ type: 'START_CYCLE' }` to `{ type: 'START_CYCLE'; dicePool?: Die[] }`
    - This allows the dispatching component to pre-compute the dice pool from character stats and pass it in the action payload

    STEP 2 - Create `src/components/Character/CharacterCreation.tsx`:
    ...
    STEP 3 - Update `src/utils/dice.ts` - modify generateDicePool():
    ...
    STEP 4 - Update `src/hooks/useGameState.tsx`:
    - In the gameReducer's START_CYCLE case: check if action.dicePool is provided. If so, use it directly as the cycle's dice pool. If not, fall back to existing condition-only generation (backward compatible).
    - In the component that dispatches START_CYCLE (CycleView or GameProvider): import useCharacter, call generateDicePool(condition, character) to compute the pool, then dispatch { type: 'START_CYCLE', dicePool: computedPool }.
  </action>
  ...
  <done>GameAction union updated with dicePool on START_CYCLE. Character creation produces a Dog with allocated stats. Cycle dice pool is generated from character stats (stat dice rolled and available for allocation).</done>
</task>
...
- GameAction START_CYCLE accepts optional dicePool parameter
</success_criteria>
