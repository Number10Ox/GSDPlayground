...
must_haves:
  ...
  artifacts:
    - path: "src/types/game.ts"
      provides: "AvailableAction interface for player-available actions"
      contains: "interface AvailableAction"
...
<task type="auto">
  <name>Task 1: Add AvailableAction type and sample actions</name>
  <files>src/types/game.ts, src/hooks/useGameState.tsx</files>
  <action>
    Add AvailableAction interface to types/game.ts (distinct from existing GameAction union which handles reducer actions):
